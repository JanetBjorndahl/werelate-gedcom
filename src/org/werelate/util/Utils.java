...
//   );
   ...
   public static final Pattern AMBIGUOUS_DATE = Pattern.compile("\\d{1,2}[-./ ]+\\d{1,2}[-./ ]+\\d{2,4}");
...
//                                                                      'Ј', 'Ð', 'Æ',  'Ł', 'Đ',  'Ø',  'Ŀ', 'І', 'Þ',       'Œ',  'Ĳ'};
   ...
   public static class ProblemsDate
   {
      private Calendar min, max;
      public ProblemsDate(String sortKey)
      {
         int year = Integer.parseInt(sortKey.substring(0,4));
         if (sortKey.length() >= 6)
         {
            int month = Integer.parseInt(sortKey.substring(4,6));
            if (sortKey.length() >= 8)
            {
               int day = Integer.parseInt(sortKey.substring(6,8));
               min = max = new GregorianCalendar(year, month, day);
            } else
            {
               min = new GregorianCalendar(year, month, 1);
               max = new GregorianCalendar(year, month, MONTH_DAYS.get(month));
            }
         } else
         {
            min = new GregorianCalendar(year, 1,1);
            max = new GregorianCalendar(year, GregorianCalendar.DECEMBER, MONTH_DAYS.get(GregorianCalendar.DECEMBER));
         }
      }
   ...
   private static int[] MONTH_OFFSETS = {0,31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365};

   private static Pattern pBeforeRegExp =  Pattern.compile("\\b(bef|before)\\b", Pattern.CASE_INSENSITIVE);
   private static Pattern pAfterRegExp =  Pattern.compile("\\b(aft|after)\\b", Pattern.CASE_INSENSITIVE);
   private static Pattern pAboutRegExp =  Pattern.compile("\\b(abt|about)\\b", Pattern.CASE_INSENSITIVE);
   private static Pattern pBCRegExp =  Pattern.compile("\\b(b\\.? ?c\\.? ?e?\\.?)\\b", Pattern.CASE_INSENSITIVE);
   private static Pattern pBCADRegExp = Pattern.compile("\\b(b\\.? ?c\\.? ?e?\\.?|a\\.? ?d\\.?|c\\.? ?e\\.?)\\b", Pattern.CASE_INSENSITIVE);

   public static int getMinDay(String key, String date) {
      if (isEmpty(key)) return 0;

      int day = 0;
      if (key.length() <= 4) {
         day = Integer.parseInt(key) * 365;
      }
      else {
         day = Integer.parseInt(key.substring(0,4)) * 365;
         if (key.length() == 6) {
            day += MONTH_OFFSETS[Integer.parseInt(key.substring(4, 6))-1];
         }
         else {
            day += MONTH_OFFSETS[Integer.parseInt(key.substring(4, 6))-1] + Integer.parseInt(key.substring(6,8)) - 1;
         }
      }
      if (date != null) {
         Matcher m = pBeforeRegExp.matcher(date);
         if (m.find()) {
            day -= 3650; // assume up to 10 years earlier
         }
         m = pAboutRegExp.matcher(date);
         if (m.find()) {
            day -= 365; // assume up to 1 year earlier
         }
         m = pBCRegExp.matcher(date);
         if (m.find()) {
            day = -day;
         }
      }
   ...
   public static int getMaxDay(String key, String date) {
      if (isEmpty(key)) return 0;

      int day = 0;
      if (key.length() <= 4) {
         day = (Integer.parseInt(key)+1) * 365 - 1;
      }
      else {
         day = Integer.parseInt(key.substring(0,4)) * 365;
         if (key.length() == 6) {
      ...
      if (date != null) {
         Matcher m = pAfterRegExp.matcher(date);
         if (m.find()) {
            day += 3650; // assume up to 10 years later
         }
         m = pAboutRegExp.matcher(date);
         if (m.find()) {
            day += 365; // assume up to 1 year later
         }
         m = pBCRegExp.matcher(date);
         if (m.find()) {
            day = -day;
   ...
   // estimate +/- 1 year for after/before date modifiers
   public static String modifyStdDate(String stdDate, String date) {
      if (date != null) {
         Matcher m = pBeforeRegExp.matcher(stdDate);
         if (m.find()) {
            return yearToString(Integer.parseInt(stdDate.substring(0,4))-1) + stdDate.substring(4);
         }
         m = pAfterRegExp.matcher(stdDate);
         if (m.find()) {
            return yearToString(Integer.parseInt(stdDate.substring(0,4))+1) + stdDate.substring(4);
         }
      }
   ...
   public static boolean isNextYear(int year, String field) {
      if (field.length() > 4) return false;
      String y = yearToString(year);
      if ((field.equals("00") && y.substring(2).equals("99")) || (field.equals("0") && y.substring(3).equals("9"))) return true;
      int newYear = parseInt(y.substring(0, 4 - field.length()) + field);
      ...
      {         
         ...
         for (int i = 0; i < fields.size(); i++) {
            ...
            if (isYear(num))
            {
               ...
               else if (i > 0 && isYear(parseInt(fields.get(i - 1))) && isNextYear(year, field)) {
                  year++; // 1699/1700
               }
         ...
         if (year == 0 && date.length() > 0) { // try again - take anything that's numeric as the year
            Matcher m = pBCADRegExp.matcher(date);
            if (m.find()) {
               month = 0;
               day = 0;
               for (int i = 0; i < fields.size(); i++) {
                  int num = parseInt(fields.get(i));
                  if (num != 0) {
                     year = num;
                     break;
                  }
...
//   public static boolean isNonNumericDate(String date) {
//      if (date == null) return true;
//
//      date = date.toLowerCase().trim();
//      return "".equals(date) || "private".equals(date) || "living".equals(date) || "unknown".equals(date) || "infant".equals(date);
//   }
